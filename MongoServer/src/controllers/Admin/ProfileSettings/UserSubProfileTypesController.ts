...
import type { UserSubProfileTypes, Users } from "../../../types/jotbox";
...
export class AdminUserSubProfileTypesController {
  private async collection() {
    ...
    return db.collection<UserSubProfileTypes>("UserSubProfileTypes");
  }
  ...
  private async nextId(collection?: Awaited<ReturnType<typeof this.collection>>) {
    ...
    const doc = await target
      ...
      .limit(1)
      .next();
    return ((doc?.userSubProfileId as number | undefined) ?? 0) + 1;
  }

  private parseIdArray(input: any): ArrayInput {
  ...
  assignUserSubProfiles = async (req: Request, res: Response) => {
    const body = req.body || {};
    ...
    try {
      ...
      if (hasUserToSubProfiles && userId !== null) {
        ...
        if (toAdd.length) {
          ...
          const docs = toAdd.map(
            (sid): UserSubProfileTypes => ({
  ...
  unassignUserSubProfiles = async (req: Request, res: Response) => {
    const body = req.body || {};
    const userId = toNumber(body.userId);
    const subProfileIds = this.parseIdArray(body.subProfileIds);
    const subProfileId = toNumber(body.subProfileId);
  ...
  getUserSubProfiles = async (req: Request, res: Response) => {
  ...
  getUsersBySubProfileId = async (req: Request, res: Response) => {
