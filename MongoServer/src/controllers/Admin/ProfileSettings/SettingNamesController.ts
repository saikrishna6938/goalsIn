...
import type { UserSettingsTypes } from "../../../types/jotbox";
...
export class AdminUserSettingsTypesController {
  ...
  private async nextId(collection?: Awaited<ReturnType<typeof this.collection>>) {
    ...
    return ((doc?.Id as number | undefined) ?? 0) + 1;
  }

  addUserSettingsType = async (req: Request, res: Response) => {
    try {
  ...
  editUserSettingsType = async (req: Request, res: Response) => {
    try {
  ...
  deleteUserSettingsType = async (req: Request, res: Response) => {
    try {
      const id = toNumber(req.params?.id);
      if (id === null) {
        return res.status(400).json({ message: "Invalid id", status: false });
  ...
  getAllUserSettingsTypes = async (_req: Request, res: Response) => {
    try {
      const collection = await this.collection();
  ...
  getSingleUserSettingsType = async (req: Request, res: Response) => {
    try {
      const id = toNumber(req.params?.id);
      if (id === null) {
        return res.status(400).json({ message: "Invalid id", status: false });
