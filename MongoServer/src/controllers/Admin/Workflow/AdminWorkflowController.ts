...
import type { Workflow, WorkflowDocumentTypes } from "../../../types/jotbox";
...
export class AdminWorkflowController {
  private async workflows() {
    ...
    return db.collection<Workflow>("Workflow");
  }

  private async workflowDocumentTypes() {
    const db = await getMongoDb();
  ...
  updateWorkflow = async (req: Request, res: Response) => {
    try {
      ...
      if (typeof req.body?.WorkflowName === "string") updates.WorkflowName = req.body.WorkflowName;
  ...
  getWorkflowDocumentTypes = async (req: Request, res: Response) => {
    try {
  ...
  addWorkflowDocumentTypes = async (req: Request, res: Response) => {
    try {
      const workflowID = toNumber(req.body?.workflowID);
      const documentTypeIds: number[] = Array.isArray(req.body?.documentTypeIds)
        ? req.body.documentTypeIds.map((id: any) => Number(id)).filter(Number.isFinite)
        : [];
      if (workflowID === null || !documentTypeIds.length) {
        return res.status(400).json({ message: "workflowID and documentTypeIds are required" });
      }
      const collection = await this.workflowDocumentTypes();
      const docs: WorkflowDocumentTypes[] = [];
      for (const DocumentTypeID of documentTypeIds) {
        docs.push({
          workflowDocumentTypeID: await nextId(collection, "workflowDocumentTypeID"),
          workflowID,
          DocumentTypeID,
        } as WorkflowDocumentTypes);
      }
  ...
  deleteWorkflowDocumentTypes = async (req: Request, res: Response) => {
    try {
      const workflowID = toNumber(req.body?.workflowID);
      const documentTypeIds: number[] = Array.isArray(req.body?.documentTypeIds)
        ? req.body.documentTypeIds.map((id: any) => Number(id)).filter(Number.isFinite)
