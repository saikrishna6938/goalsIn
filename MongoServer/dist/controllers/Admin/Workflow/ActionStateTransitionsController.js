...
const nextId = async (collection) => {
    const doc = await collection
        ...
        .limit(1)
        .next();
    return (doc?.transitionId ?? 0) + 1;
};
class ActionStateTransitionsController {
    constructor() {
        this.getTransitionsByAction = async (req, res) => {
            try {
        ...
        this.addTransition = async (req, res) => {
            try {
        ...
        this.updateTransition = async (req, res) => {
            try {
                ...
                const toStateId = toNumber(req.body?.toStateId);
                if (actionId !== null)
                    updates.actionId = actionId;
                if (fromStateId !== null)
                    updates.fromStateId = fromStateId;
                if (toStateId !== null)
                    updates.toStateId = toStateId;
        ...
        this.deleteTransition = async (req, res) => {
            try {
                const transitionId = toNumber(req.body?.transitionId);
                if (transitionId === null) {
                    return res.status(400).json({ message: "transitionId is required" });
