...
class DocumentTypeController {
    constructor() {
        ...
        this.updateDocumentType = async (req, res) => {
            try {
        ...
        this.getDocumentType = async (req, res) => {
            try {
                const documentTypeId = toNumber(req.params.documentTypeId);
                if (documentTypeId === null) {
                    return res.status(400).json({ status: false, message: "Invalid documentTypeId" });
        ...
        this.getDocumentTypesByUserType = async (req, res) => {
            try {
        ...
        this.addDocumentTypeObject = async (req, res) => {
            try {
                const payload = req.body || {};
                ...
                const documentTypeObjectId = toNumber(payload.documentTypeObjectId) ?? (await this.nextId(collection, "documentTypeObjectId"));
                const doc = {
                    documentTypeObjectId,
                    name: normalizeString(payload.name) ?? "",
                    description: normalizeString(payload.description) ?? "",
                    documentTypeObject: typeof payload.documentTypeObject === "string"
        ...
        this.updateDocumentTypeObject = async (req, res) => {
            try {
        ...
        this.deleteDocumentTypeObject = async (req, res) => {
            try {
                const documentTypeObjectId = toNumber(req.params.documentTypeObjectId);
                if (documentTypeObjectId === null) {
                    return res.status(400).json({ status: false, message: "Invalid documentTypeObjectId" });
        ...
        this.getDocumentTypeObjectById = async (req, res) => {
            try {
                const documentTypeId = toNumber(req.params.documentTypeId);
                if (documentTypeId === null) {
                    return res.status(400).json({ status: false, message: "Invalid documentTypeId" });
                ...
                const collection = await this.documentTypeObjects();
                const objectDoc = await collection.findOne({ documentTypeObjectId: doc.documentTypeObjectId }, { projection: { _id: 0 } });
                if (!objectDoc) {
                    return res.status(404).json({ status: false, message: "DocumentTypeObject not found" });
                }
        ...
        this.getDocumentTypeUsers = async (req, res) => {
            try {
                ...
                const documentTypes = await this.documentTypes();
                const documentType = await documentTypes.findOne({ documentTypeId }, { projection: { documentTypeRoles: 1, _id: 0 } });
                if (!documentType) {
        ...
        this.getDocumentTypeByRoles = async (req, res) => {
            try {
        ...
        this.getDocumentTypeUsersLegacy = this.getDocumentTypeUsers;
    }
    async documentTypes() {
