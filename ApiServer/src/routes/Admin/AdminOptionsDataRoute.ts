...
class AdminOptionsRoute {
  ...
  config(): void {
    const base = `${keys.basePath}administrator/options`;

    this.router.post(`${base}`, (req, res) =>
      adminOptionsController.createOption(req, res)
    );

    this.router.get(`${base}`, (req, res) =>
      adminOptionsController.getOptions(req, res)
    );

    this.router.delete(`${base}/:optionId`, (req, res) =>
      adminOptionsController.deleteOption(req, res)
    );

    this.router.put(`${base}/:optionId`, (req, res) =>
      adminOptionsController.updateOption(req, res)
    );

    this.router.get(`${base}/label/:optionName`, (req, res) =>
      adminOptionsController.getOptionDataByLabelName(req, res)
    );
    this.router.get(`${base}/:optionId`, (req, res) =>
      adminOptionsController.getOptionsByID(req, res)
    );

    this.router.get(`${base}/by-label`, (req, res) =>
      adminOptionsController.getOptionsDataByValueLabel(req, res)
    );
