...
const UserForm: React.FC<UserFormProps> = ({ currentObj, onReload, data }) => {
    ...
    const fetchData = async () => {
      try {
        ...
        const rightRoles = allRoles.filter(
          (role) => Number(role.documentTypeObjectId) === Number(currentObj?.id)
        );
        ...
        const leftRoles = allRoles.filter(
          (role) =>
            Number(role.documentTypeObjectId) !== Number(currentObj?.id) &&
  ...
  const fetchFormData = async () => {
    ...
    try {
      const response = await api.get(`administrator/forms/${currentObj.id}`);
  ...
  const handleOnSubmit = async (values: any) => {
    try {
      ...
      const url = `administrator/forms`;

      const payload = isEdit
        ? Object.keys(values).reduce(
            (acc: any, key) => {
              if (values[key] !== currentObj[key]) {
                acc[key] = values[key];
              }
              return acc;
            },
            { documentTypeObjectId: currentObj.id }
          )
        : {
            ...values,
  ...
  const onSubmit = async (added: any[], removed: any[]) => {
    try {
      ...
      for (const role of added) {
        const payload = { documentTypeObjectId: currentObj?.id };
      ...
      for (const role of removed) {
        const payload = { documentTypeObjectId: -1 };
        const response = await api.put(
          `administrator/document-types/${role.roleNameId}`,
          {
            body: payload,
